import logging
from llm import llm
from agents.schemas import RiskFactorsResult
from agents.entity_agent import get_flat_values

logger = logging.getLogger(__name__)

_structured_llm = llm.with_structured_output(RiskFactorsResult)
...
def _compute_score(investigation: dict, entities: dict, intelligence: dict) -> tuple[int, list[str]]:
    """Deterministically score a complaint 0-100. entities must be flat string lists."""
    score = 0
    triggered = []
    ...
    for kw in ["arrest", "kidnap", "raid", "warrant", "case", "legal"]:
        if kw in scam_type:
            score += 15
            triggered.append(f"Extreme psychological pressure tactic: '{scam_type}'")
            break
...
Scam type: {investigation.get('scam_type', 'Unknown')}

Write 2-4 concise risk factor statements explaining this score. One sentence each.
"""

    result: RiskFactorsResult = _structured_llm.invoke(prompt)
    risk_factors = result.risk_factors or triggered_signals
